...
export const DSARStatusBadge = ({ status, className }: Props) => {
  ...
  return (
    <View className={cn('px-2 py-0.5 rounded-full border items-center justify-center', getStyles().split(' ').filter(s => s.startsWith('bg-') || s.startsWith('border-')).join(' '), className)}>
